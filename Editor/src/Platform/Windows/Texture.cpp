...
Texture::Texture(glm::vec4 color, const char* path)
{
    m_Path = path;
    LoadColor(color);
}
...
bool Texture::LoadTexture(const char* filename)
{
    ...
    if (!textureData && !m_TextureNotFound) {
        ...
        LoadColor(ErrorColorRGBA); // Load purple
        return false;
    }
...
bool Texture::LoadColor(glm::vec4 color)
{
    m_Width = 1;
    m_Height = 1;
    m_Channels = 4; // RGBA
    ...
    textureData[3] = static_cast<unsigned char>(color.a * 255);
